...
from utils.db_api.models.func_model_user import check_for_user_sub, get_user, add_user, del_user
...
async def menu_user_main(query: types.CallbackQuery):
    await query.answer()
...
async def callback_vote_action(query: types.CallbackQuery):
    if not get_user(query.from_user.id):
